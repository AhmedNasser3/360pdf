...
const Optimize = () => {
  ...
  // Send file to backend for optimization
  const handleOptimize = async () => {
    if (!selectedFile) {
      toast.error("Please select a PDF file to optimize.");
      return;
    }
    ...
    setDownloadUrl("");
    setErrorMsg("");
    toast.loading("Optimizing your PDF file...", { id: "optimize" });

    const formData = new FormData();
    formData.append("pdf", selectedFile);

    try {
      const res = await axios.post("/api/optimize-pdf", formData, {
      ...
      if (data && data.download_url) {
        ...
        toast.success("PDF optimized successfully!", { id: "optimize" });
      } else {
      ...
      const msg = err.response?.data?.error || "An error occurred during optimization.";
      setErrorMsg(msg);
      toast.error(msg, { id: "optimize" });
    } finally {
      setLoading(false);
  ...
  const handleDirectDownload = () => {
    ...
    link.setAttribute("download", "optimized.pdf");
        ...
        <div className="convert_box_container">
          <div className="convert_box_content">
            <div className="convert_box_titles">
              <h1>Optimize PDF File</h1>
              <p>
                Upload a PDF file and optimize it to reduce size and improve performance instantly.
              </p>
            </div>
          </div>
        </div>
          ...
          {downloadUrl && (
            <div className="download-link">
              <button onClick={handleDirectDownload}>
                <FaDownload /> Download Optimized File
              </button>
            </div>
          )}
        </div>
