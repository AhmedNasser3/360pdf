...
class OcrController extends Controller
{
     public function ocrPdf(Request $request)
    {
        try {
            ...
            // عملية OCR
            $ocrTask = (new Task('pdf/ocr', 'task-1'))
                ->set('input', ['import-1'])
                ->set('engine', 'ocrmypdf')
                ->set('auto_orient', false)
                ->set('language', ['eng']) // اللغة الافتراضية
                ->set('filename', 'OCR.pdf')
                ->set('timeout', 10);
            ...
            if ($downloadUrl) {
                $ocrFile = file_get_contents($downloadUrl);
                $localPath = 'ocr/' . uniqid() . '_ocr.pdf';
                Storage::disk('public')->put($localPath, $ocrFile);
            ...
            return response()->json([
                'message' => '✅ تم استخراج النصوص (OCR) من الملف بنجاح',
                'download_url' => $downloadUrl,
                'local_path' => asset('storage/' . $localPath),
            ]);
        } catch (\Exception $e) {
            Log::error('❌ خطأ أثناء OCR PDF', ['error' => $e->getMessage()]);
            return response()->json([
                'error' => $e->getMessage(),
            ], 500);
        }
